...
class GuacdClient {
    ...
    sendHandshakeReply() {
        ...
        if (this.clientConnection.connectionSettings.connection.audio) {
            this.sendOpCode(['audio', this.clientConnection.connectionSettings.connection.audio]);
        } else {
            this.sendOpCode(['audio']);
        }

        if (this.clientConnection.connectionSettings.connection.video) {
            this.sendOpCode(['video', this.clientConnection.connectionSettings.connection.video]);
        } else {
            this.sendOpCode(['video']);
        }

        if (this.clientConnection.connectionSettings.connection.image) {
            this.sendOpCode(['image', this.clientConnection.connectionSettings.connection.image]);
        } else {
    ...
    static parseOpCodeAttribute(opCodeAttribute) {
        let parsedAttribute = opCodeAttribute.substring(opCodeAttribute.indexOf('.') + 1, opCodeAttribute.length);
